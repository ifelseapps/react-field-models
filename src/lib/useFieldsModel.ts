...
import { FieldsExtra, FieldsInitial, Mode } from './types';
...
export function useFieldsModel<TInitial extends FieldsInitial>(initial: TInitial): FieldsExtra<TInitial> {
      ...
      return getObjectKeys(initial).reduce<FieldsExtra<TInitial>>((result, name) => {
        ...
        return {
          ...
          [name]: {
            ...
            errors: isFunction(current.onValidate) ? current.onValidate(current.value) : null,
            onChange: isFunction(current.onChangeAsync) ? current.onChangeAsync(onChange, setMode) : onChange,
            setMode,
          }
        };
      }, {} as FieldsExtra<TInitial>)
